...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setAppName("wikipedia").setMaster("local[3]")
  ...
  /** Returns the number of articles on which the language `lang` occurs.
    ...
    */
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.filter(a => contains(lang, a.text)).count().toInt

  private def contains(param: String, source: String): Boolean = source.split(" ").contains(param)
  ...
  def findLanguages(langs: List[String], article: WikipediaArticle) = langs.filter(contains(_, article.text))
